...
import { Plus, Pencil, PlusCircle, Check, Info } from "lucide-react";
import {
  getUserSettings,
  updateUserSettings,
  addCategory,
...
export default function SettingsPage() {
  ...
  const [showGamification, setShowGamification] = useState<boolean>(true);
  const [savingGeneral, setSavingGeneral] = useState(false);

  // Category modal state
  const [modalOpen, setModalOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<string | null>(null); // name of category being edited
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      setSettings(s);
      setBudgetStartDay(s.budgetStartDay || 1);
      setShowGamification(s.showGamification !== false);
    } catch (error) {
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingCategory) {
        res = await updateCategory(editingCategory, {
          name: catName.trim(),
          icon: catIcon,
          color: catColor,
        });
          ...
          <form onSubmit={handleSaveCategory} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled={editingCategory?.toLowerCase() === "other"}
              />
              {editingCategory?.toLowerCase() === "other" && (
